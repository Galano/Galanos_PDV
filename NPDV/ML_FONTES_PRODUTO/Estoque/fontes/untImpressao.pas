...
interface
  Uses Windows, Messages, Sysutils, Classes, Graphics, Dialogs, IniFiles, Db,
       Forms, StdCtrls, ComCtrls, Mask, DBCtrls, ExtCtrls, Udm, UFrmRelVendas;
...
begin
    Try
      ...
      linhaCupom := 14;
      FrmRelVendas.Q_Itens_Venda.First;

      while not FrmRelVendas.Q_Itens_Venda.Eof do
      begin
        inc(linhaCupom);
        Say(linhaCupom,00,LinhaAtual,ArqPrn,Copy(FrmRelVendas.Q_Itens_VendaNOME_PRO.AsString,1, ndeColunas));
        inc(linhaCupom);
        Say(linhaCupom,00,LinhaAtual,ArqPrn,Padr(FormatFloat('#,###,###,##0.000', FrmRelVendas.Q_Itens_VendaQUANT_ITEM.Value) + '  X  ' + FormatFloat('#,###,###,##0.00', FrmRelVendas.Q_Itens_VendaPRECO_VAREJO.Value) + '     ' + FormatFloat('#,###,###,##0.00', (FrmRelVendas.Q_Itens_VendaQUANT_ITEM.Value * FrmRelVendas.Q_Itens_VendaVALOR_ITEM.Value)),ndeColunas));
        FrmRelVendas.Q_Itens_Venda.Next;
      end;
      ...
      {inc(linhaCupom);
      Say(linhaCupom,00,LinhaAtual,ArqPrn,Padr('SUB-TOTAL: R$ ' + FrmVendas.lbTotalCom.Caption,48));
      inc(linhaCupom);
      Say(linhaCupom,00,LinhaAtual,ArqPrn,Padr('DESCONTO: R$ ' + formatfloat('##,###,##0.00', FrmVendas.EdtDesconto.Value),48));}

      inc(linhaCupom);
      Say(linhaCupom,00,LinhaAtual,ArqPrn,Padr('TOTAL CUPOM: R$ ' + formatfloat('##,###,##0.00', DmDados.QRelVendasTOTAL.Value),ndeColunas));
